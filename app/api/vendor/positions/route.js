...
export async function GET(request) {
  try {
    ...
    const list = await prisma.vendorPosition.findMany({
      where: { adminUserId: vendorId },
      orderBy: { name: "asc" },
    })
    return NextResponse.json(list)
  } catch (error) {
    console.error("Vendor positions GET:", error)
...
export async function POST(request) {
  try {
    ...
    const created = await prisma.vendorPosition.create({
      data: { name, adminUserId: vendorId },
    })
    return NextResponse.json(created, { status: 201 })
  } catch (error) {
    console.error("Vendor positions POST:", error)
